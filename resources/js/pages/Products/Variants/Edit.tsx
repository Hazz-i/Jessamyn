...
export default function EditProductVariant({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    put(route('product.variants.update', [productId, variant.id]), {
      ...
      onSuccess: () => {
        ...
        reset();
        clearErrors();
        document.body.style.overflow = 'auto'; // pastikan scroll balik
      },
    });
  };

  // jaga-jaga kalau modal dibuka → lock scroll
  useEffect(() => {
    if (open) {
    ...
    // cleanup supaya body overflow gak nyangkut
    return () => {
      document.body.style.overflow = 'auto';
    };
  }, [open]);
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-[22rem] p-6 bg-white rounded-xl shadow-lg z-50">
        <DialogTitle>Edit Variant</DialogTitle>
        <DialogDescription>
          Update the product variant below.
        </DialogDescription>
        <form onSubmit={handleSubmit} className="grid gap-4 py-2">
          <div className="grid gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="min-w-[12rem] z-[60]">
                {data.category == 'Bundle' ? (
                  <DropdownMenuItem onClick={() => setData('variant', '60ml-25ml-100ml')}>
                    60ml-25ml-100ml
                  </DropdownMenuItem>
                ) : (
                  <>
